...
def _assert_can_write_mark(request, student: Student, subject_id) -> Response | None:
    ...
    if p.role == UserProfile.Role.SUBJECT_TEACHER:
        if not p.assigned_subject_id or sid != int(p.assigned_subject_id):
            return _forbidden('You can only enter marks for your assigned subject.')
...
def subject_performance_analytics(request):
    ...
    if prof and prof.role == UserProfile.Role.SUBJECT_TEACHER and prof.assigned_subject:
        sn = prof.assigned_subject.name
        subjects = [s for s in subjects if s['subject'] == sn]
    ...
    if compare_class and compare_class != class_name:
        ...
        if prof and prof.role == UserProfile.Role.SUBJECT_TEACHER and prof.assigned_subject:
            sn = prof.assigned_subject.name
            sub_b = [s for s in sub_b if s['subject'] == sn]
        compare_payload = {
    ...
    if include_trend:
        ...
        for slug in ('UNIT_TEST', 'SEMESTER'):
            if ExamType.objects.filter(slug=slug).exists():
                ...
                if prof and prof.role == UserProfile.Role.SUBJECT_TEACHER and prof.assigned_subject:
                    sn = prof.assigned_subject.name
                    tr = [s for s in tr if s['subject'] == sn]
...
def subject_list(request):
    ...
    if prof and prof.role == UserProfile.Role.SUBJECT_TEACHER and prof.assigned_subject_id:
        qs = qs.filter(pk=prof.assigned_subject_id)
